    ...
    <div className="min-h-0 flex-1 overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
      <div className="h-full overflow-y-auto overflow-x-hidden [scrollbar-color:#cbd5e1_transparent] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:border-2 [&::-webkit-scrollbar-thumb]:border-white [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb:hover]:bg-slate-400 [&::-webkit-scrollbar-track]:bg-transparent">
        <Table className="w-full table-fixed">
          ...
          <TableHeader className="sticky top-0 z-10 border-b border-slate-100 bg-white">
            <TableRow className="h-14 hover:bg-transparent">
              <TableHead className="px-8 py-4 text-left text-sm font-semibold text-slate-500">ID</TableHead>
              <TableHead className="px-8 py-4 text-left text-sm font-semibold text-slate-500">Item</TableHead>
              <TableHead className="px-8 py-4 text-left text-sm font-semibold text-slate-500">Image</TableHead>
              <TableHead className="px-8 py-4 text-left text-sm font-semibold text-slate-500">Quantity</TableHead>
              <TableHead className="px-8 py-4 text-center text-sm font-semibold text-slate-500">Actions</TableHead>
            </TableRow>
          </TableHeader>
          ...
          <TableBody>
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <TableRow key={item.id} className="h-24 border-b border-slate-100 last:border-0 hover:bg-slate-50/50">
                  <TableCell className="px-8 py-5 text-left text-lg font-semibold text-slate-950">
                    {item.id}
                  </TableCell>
