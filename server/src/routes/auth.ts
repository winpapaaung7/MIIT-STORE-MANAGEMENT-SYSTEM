...
const authRateLimitWindowMs = positiveEnvNumber("AUTH_RATE_LIMIT_WINDOW_MINUTES", 15) * 60_000;
const loginIpLimit = positiveEnvNumber("AUTH_LOGIN_IP_LIMIT", 30);
const loginEmailLimit = positiveEnvNumber("AUTH_LOGIN_EMAIL_LIMIT", 10);
function rateLimit(req: Request, res: Response, key: string, limit: number) {
  const now = Date.now();
  const values = (attempts.get(key) ?? []).filter((time) => time > now - authRateLimitWindowMs);
  if (values.length >= limit) {
    const retryAfter = Math.ceil((values[0] + authRateLimitWindowMs - now) / 1000);
    res.set("Retry-After", String(retryAfter));
    res.status(429).json({ ok: false, message: "Too many requests. Please try again later.", retryAfter });
    return false;
  }
  values.push(now);
...
export function createAuthRouter(prisma: any) {
  const router = Router();
  router.post("/login", async (req, res) => { const email = normalizeEmail(req.body?.email), password = typeof req.body?.password === "string" ? req.body.password : "", ip = clientIp(req); if (!rateLimit(req, res, `login:ip:${ip}`, loginIpLimit) || !rateLimit(req, res, `login:email:${email}`, loginEmailLimit)) return; if (!email || !password) return res.status(400).json({ ok: false, message: "Email and password are required" }); try { const user = await prisma.users.findUnique({ where: { email }, include: userInclude }); if (!user || user.status.toLowerCase() !== "active" || !(await verifyPassword(password, user.password_hash))) return res.status(401).json({ ok: false, message: "Invalid email or password" }); try { toAuthenticatedUser(user); } catch { return res.status(403).json({ ok: false, message: "Account role assignment is invalid" }); } return res.json(user.two_step_enabled ? await createAndSendChallenge(prisma, user, ip) : await issueSession(prisma, user, res)); } catch (error) { console.error("Sign in request failed", error instanceof Error ? error.message : "unknown error"); return res.status(503).json({ ok: false, message: "Sign in is temporarily unavailable" }); } });
